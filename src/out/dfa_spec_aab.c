...
int m_opt_anyaab(char* s) {
  ...
  /*@
    ...
    loop invariant (id==0 ==> \forall int j; 0 <= j < i ==> !match_aab(s+j));
    loop invariant (id==1 ==> i>=1 && \forall int j; 0 <= j < i-1 ==> !match_aab(s+j));
    loop invariant (id==2 ==> i>=2 && \forall int j; 0 <= j < i-2 ==> !match_aab(s+j));
