...
class BipTests extends TestSuite {
  ...
  trait BipProgram extends Dsl {
    def req: Rep[Int] => Rep[Boolean]
    def cypher_fun: Rep[Int] => Rep[Int]
    def decypher_fun: Rep[Int] => Rep[Int]
    ...
    val cypher = gencypher("cypher", req, cypher_fun)

    val decypher = gencypher("decypher", req, decypher_fun)
  ...
  test("cesar1") {
    trait CesarProgram extends BipProgram {
      override def req = {s : Rep[Int] => 0 <= s && s <= 26 }
      override def cypher_fun = { s: Rep[Int] => if (s==26) 0 else s+1 }
      override def decypher_fun = { s: Rep[Int] => if (s==0) 26 else s-1 }
    }
    check("cesar1", (new CesarProgram with Impl).code)
  }
}
